...
class PointCloud:
    ...
    def graph_statistical_removal_and_PCA_variance(self, nb_neighbor_upper: int, nb_neighbors_samples: int, std_dev_upper: float,
                                               std_dev_samples: int, nb_neighbor_lower: int = 1,
                                               std_dev_lower: float = .000001) -> None:
        ...
        for i in range(3):
            ...
            ax.view_init(azim=160, elev=30)
        plt.show()
        print(np.nanmin(self.explained_variance_ratio_data[2, :, :]))
        return

    def graph_statistical_removal_and_average_distance(self, nb_neighbor_upper: int, nb_neighbors_samples: int, std_dev_upper: float,
                                                   std_dev_samples: int, nb_neighbor_lower: int = 1,
                                                   std_dev_lower: float = .000001) -> None:
    ...
    def evaluate_filter_parameters(self, nb_neighbors = 0, std_ratio = 0, method:str = None) -> None:
    ...
    def create_summary_subplots(self, method: str):
        """Creates a matplotlib figure with 4 subplots. The first subplot is the original point cloud. The second subplot is the noisy point cloud.
        The third subplot is a graph representing output from either the average distance method or 
        the PCA method, depending on which method is selected. The fourth subplot is the filtered point cloud.
